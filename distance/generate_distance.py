...
def parse_bc():
    ...
    parser_prog = Path(__file__).parent / PARSER_DIR_NAME / PARSER_NAME
...
def calculate_cg():
    ## check python script
    cal_cg_script = Path(__file__).parent / CALCULATOR_DIR_NAME / CAL_CG_NAME
...
def calculate_cfg():
    ## check python script
    cal_cfg_script = Path(__file__).parent / CALCULATOR_DIR_NAME / CAL_CFG_NAME
...
def exit_handler():
    py_cache_dir = Path(__file__).parent / CALCULATOR_DIR_NAME / PY_CACHE_DIR_NAME
...
def main():
    ...
    if get_step() == 0:
        info_step(f"Parsing bitcode file of {BIN_NAME}")
        parse_bc()
        next_step()
    if get_step() == 1:
